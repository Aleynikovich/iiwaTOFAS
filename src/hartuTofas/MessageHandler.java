...
public class MessageHandler {
	...
	public class Command {
		...
		public Command(String[] parts) {
			...
			if (rawActionType > 100) {
				...
				this.actionType = rawActionType - 100; // normalize to actual
														// program ID
			} else {
		...
		try {
			...
			if (!cmd.programCall) {
				...
				case PTP_AXIS_C:
					...
					return handlePTPAxis(cmd.actionType, cmd.numPoints,
							cmd.targetPoints, cmd.id);
				case PTP_FRAME:
				case PTP_FRAME_C:
					return handlePTPFrame(cmd.numPoints, cmd.targetPoints,
							cmd.id);
				case LIN_FRAME:
					return handleLINFrame(cmd.numPoints, cmd.targetPoints,
							cmd.id);
				default:
	...
	private String handlePTPAxis(int moveType, int numPoints,
			String targetPoints, String id) {
		// Process the PTP_AXIS or PTP_AXIS_C command
		List<String> jointPositionGroups = Arrays.asList(targetPoints
				.split(","));
	...
	private String handlePTPFrame(int numPoints, String targetPoints, String id) {
		...
		try {
			...
			System.out
					.println("Coordinate values are incomplete for ID: " + id);
